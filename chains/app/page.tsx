...
export default function Home() {
  ...
  const [rejectedAliases, setRejectedAliases] = useState<Set<string>>(new Set())
  const [pendingApprovals, setPendingApprovals] = useState<Map<string, Alias>>(new Map())
  ...
  // Derived state: filter out rejected aliases
  const activeAliases = useMemo(() => {
    if (!results) return []
    return results.files[0]?.aliases_used.filter(
      alias => !rejectedAliases.has(alias.source_name)
    ) || []
  }, [results, rejectedAliases])

  // Derived state: validation rate
  const validationRate = useMemo(() => {
    if (!results || !results.files[0]) return 0
    const validationStatus = results.files[0].validation_status
  ...
  // Derived state: match rate (simple placeholder)
  const matchRate = useMemo(() => {
    if (!results || !results.files[0]) return 0
    const { confirmation_rows, booking_matches_rows } = results.files[0]
    if (confirmation_rows.length === 0) return 0
    return Math.round((booking_matches_rows.length / confirmation_rows.length) * 100)
  }, [results])

  // Handler: Approve alias
  const handleApproveAlias = async (alias: Alias, level: 'global' | 'counterparty') => {
    try {
      // API call to save alias
      const response = await fetch('/api/aliases/save', {
        method: 'POST',
      ...
      setPendingApprovals(prev => {
        ...
        updated.set(alias.source_name, alias)
        return updated
      })

      toast.success(`Alias saved at ${level} level`)
  ...
  // Handler: Reject alias
  const handleRejectAlias = (alias: Alias) => {
    setRejectedAliases(prev => {
      const updated = new Set(prev)
      updated.add(alias.source_name)
      return updated
    })
  ...
  const handleFieldMapping = (unknownField: string, canonicalField: string) => {
    setFieldMappings(prev => {
      const updated = new Map(prev)
      updated.set(unknownField, canonicalField)
      return updated
    })
    toast.success(`Mapped "${unknownField}" to "${canonicalField}"`)
    setShowReprocessButton(true)
  }

  // Handler: Reprocess files with updated rules
  const handleReprocess = async () => {
    if (!singleFile && multipleFiles.length === 0) return
    ...
    try {
      ...
      // Add field mappings
      formData.append('field_mappings', JSON.stringify(Object.fromEntries(fieldMappings)))

      // Add approved aliases
      const approvedAliases = Array.from(pendingApprovals.values())
      formData.append('approved_aliases', JSON.stringify(approvedAliases))
      ...
      toast.success('Files reprocessed successfully')
    } catch (err) {
      setError(err instanceof Error ? err.message : 'An error occurred during reprocessing')
  ...
  // Handler: Field selection for filtering
  const handleFieldClick = (field: string) => {
    setSelectedField(prev => prev === field ? null : field)
  }

  const handleSaveAlias = async (alias: Alias, saveLevel: 'global' | 'counterparty') => {
    await handleApproveAlias(alias, saveLevel)
  }
      ...
      <main className="min-h-screen bg-background py-8 px-4">
        <div className="max-w-7xl mx-auto">
          <ResultsView
            ...
            onRejectAlias={handleRejectAlias}
