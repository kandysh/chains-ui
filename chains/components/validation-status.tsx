...
export function ValidationStatusComponent({ status, onFieldClick, selectedField }: ValidationStatusProps) {
            ...
            {entries.map(([field, valid]) => (
              <button
                ...
                className={`relative p-4 rounded-lg border-2 transition-all duration-200 text-left w-full ${
                  ...
                  valid
                    ? 'border-green-500/20 bg-green-500/5 hover:border-green-500/40 hover:shadow-md cursor-pointer'
                    : 'border-destructive/20 bg-destructive/5 hover:border-destructive/40 cursor-pointer'
                }`}
                title="Click to filter issues by this field"
              >
