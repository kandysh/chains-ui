...
import { UnknownFieldMapper } from './unknown-field-mapper'
...
export function IssuesPanel({
  ...
  // Filter out ignored unknown fields
  const visibleUnknownFields = useMemo(
    () => unknownFields.filter(field => !ignoredFields.has(field)),
  ...
  // Simple AI suggestion logic based on field name similarity
  const getSuggestedField = (unknownField: string): string | undefined => {
    const normalized = unknownField.toLowerCase().replace(/[_\s-]/g, '')

    const suggestions: Record<string, string> = {
      'terminationdate': 'expiry date',
      'maturitydate': 'expiry date',
      'enddate': 'expiry date',
      'startdate': 'strike date',
      'effectivedate': 'strike date',
      'tradedate': 'strike date',
      'buysell': 'direction',
      'side': 'direction',
      'indexname': 'index',
      'underlying': 'index',
  ...
  // Filter unmatched values by selected field
  const filteredUnmatchedValues = useMemo(() => {
    if (!selectedField) return unmatchedValues
    return selectedField in unmatchedValues
      ? { [selectedField]: unmatchedValues[selectedField] }
      : {}
  }, [unmatchedValues, selectedField])

  const hasUnmatched = Object.keys(filteredUnmatchedValues).length > 0
  const hasUnknown = visibleUnknownFields.length > 0
  const hasIssues = hasUnmatched || hasUnknown

  // Scroll to panel when field is selected
  useEffect(() => {
  ...
  return (
    <Card ref={panelRef} className="border-2 border-yellow-500/30 bg-gradient-to-br from-yellow-500/5 via-transparent to-orange-500/5">
        ...
        <Tabs defaultValue="unmatched" className="w-full">
              ...
              {selectedField && (
                <div className="mb-3 p-2 bg-primary/10 border border-primary/20 rounded-lg">
                  <p className="text-xs text-muted-foreground">
                    Filtered by: <span className="font-semibold text-foreground">{selectedField}</span>
                  </p>
                </div>
              )}
              {Object.entries(filteredUnmatchedValues).map(([field, values]) => (
          ...
          {hasUnknown && (
            <TabsContent value="unknown" className="space-y-3 mt-4">
              <div className="bg-gradient-to-r from-blue-500/5 to-purple-500/5 border-2 border-blue-500/20 rounded-xl p-5 mb-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-foreground">
                      {visibleUnknownFields.length} Unrecognized Field{visibleUnknownFields.length !== 1 ? 's' : ''} Detected
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">
                      Map these fields to canonical fields or ignore them to improve future processing
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                {visibleUnknownFields.map((field, i) => (
                  <UnknownFieldMapper
                    key={`unknown-mapper-${i}`}
                    unknownField={field}
                    canonicalFields={canonicalFields}
                    suggestedField={getSuggestedField(field)}
                    onMap={onFieldMapping || (() => {})}
                    onIgnore={handleIgnoreField}
                  />
                ))}
              </div>
            </TabsContent>
